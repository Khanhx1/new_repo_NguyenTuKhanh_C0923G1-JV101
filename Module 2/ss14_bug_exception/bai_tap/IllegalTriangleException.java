...
public class IllegalTriangleException {
    ...
    public static void checkConditionTriangleException(double edge1, double edge2, double edge3) {
        if (edge1 <= 0 || edge2 <= 0 || edge3 <= 0) {
            throw new CustomNumberFormatException("Edge of triangle must be a positive number.");
        }
        if (edge1 + edge2 <= edge3 || edge1 + edge3 <= edge2 || edge2 + edge3 <= edge1) {
            throw new CustomNumberFormatException("The input edge does not meet the triangle's conditions.");
        }
    }
